...
class Initializer(websauna.system.Initializer):
    ...
    def configure_static(self):
        ...
        self.config.registry.static_asset_policy.add_static_view('{{ cookiecutter.repo_name }}-static', '{{ cookiecutter.repo_name }}:static')

    def configure_templates(self):
        """Include our package templates folder in Jinja 2 configuration."""
        super(Initializer, self).configure_templates()

        self.config.add_jinja2_search_path('{{ cookiecutter.repo_name }}:templates', name='.html', prepend=True)  # HTML templates for pages
        self.config.add_jinja2_search_path('{{ cookiecutter.repo_name }}:templates', name='.txt', prepend=True)  # Plain text email templates (if any)
        self.config.add_jinja2_search_path('{{ cookiecutter.repo_name }}:templates', name='.xml', prepend=True)  # Sitemap and misc XML files (if any)
